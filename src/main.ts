...
function todayView(): string {
  ...
  if (!state.chores.length) return `
    <section class="empty-state"><div><p class="eyebrow">${esc(state.householdName)} · RULEBOOK READY</p><h2>Add the first household rule</h2><p>Define a recurring chore, its real effort, and whether ownership rotates or stays fixed.</p><button class="primary" data-action="add-chore">${icon('plus')} Add a chore</button></div><img src="/assets/house-signal.webp" width="768" height="512" alt="" decoding="async"></section>`;
...
function onboarding(): string {
  return `<section class="onboarding">
    ...
    <figure class="hero-art"><picture><source srcset="/assets/house-signal.webp" type="image/webp"><img src="/assets/house-signal.png" width="768" height="512" alt="A pixel-art cutaway home where glowing routes connect dishes, laundry, plants, and a broom" decoding="async" fetchpriority="high"></picture><figcaption>One home. Visible rules. Shared understanding.</figcaption></figure>
  </section>`;
}
